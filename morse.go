...
func main() {
	...
	fmt.Println("-DAH .DIT")
	fmt.Println("Find hidden morse code messages in... things!!")
	fmt.Println("Enter a string as an argument from the command line and optionally define the\ncharacters to be interpreted as the following:")
	fmt.Printf("dash 'dah' (default: '%c')\n", dahChar)
	fmt.Printf("dot 'dit' (default: '%c')\n", ditChar)
	fmt.Printf("space [to delimit CHARACTERS not words] (default: '%c')\n", space)
	fmt.Printf("word [to print a space between words] (default: '%c')\n", word)
